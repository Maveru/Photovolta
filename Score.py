...
def calcular_puntuacion(puntos):
    """
    Calcula la puntuación para una lista de puntos (latitud, longitud)
    ...
    for i in range(len(puntos)):
        ...
        for j in range(len(puntos)):
           ...
           # print(distancia, " entre punto", i , "y punto ", j)
        ...
        puntuacion.append(puntuacion_punto)
    print(puntuacion)
    return puntuacion
...
def AsignarPuntos(usuario):
    ...
    for entrada in db.session.query(DatoPersona).filter_by(username=usuario.email).all():
        puntos.append((entrada.latitud, entrada.longitud)) 
    puntuaciones = calcular_puntuacion(puntos)
    puntosSensores = 0
    ...
    for puntuacion in puntuaciones:
        puntosMan = puntosMan+puntuacion
    
    return puntosMan*0.75 + puntosSensores*0.25
    


def calcular_puntuacion_entrada(entrada, puntos):
    """
